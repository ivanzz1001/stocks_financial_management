...
- 计算差离值(DIFF): 将短期指数移动平均线减去长期指数移动平均线，得到差离值

  ```txt
  DIFF = 12日EMA - 26日EMA
  ```

- 计算DEA线(DIFF的9日指数移动平均线): 取前9个周期的差离值数据，计算加权平均值，得到DEA线
...
- 计算MACD柱状图: 将DIFF减去DEA线，得到MACD柱状图
...
MACD柱子面积是指MACD指标中`红色`(多头)或`绿色`(空头）柱状图在特定时间段内与零轴所围成的区域面积总和。它并非MACD指标的原始组成部分，而是投资者为量化动能强度而衍生出的分析概念，其核心逻辑是：面积越大，代表多空双方在该周期内释放的总能量越强。

#### 3.1.1 计算方法

MACD柱子面积的计算主要基于对柱状图数值的累加，常用方法包括：

- 直接累加法（最常用）‌

  将连续同色柱状图（红柱或绿柱）的数值（高度）进行代数相加。红柱面积为正值累加，绿柱面积为负值累加（或取绝对值后单独计算）。例如，一段红柱数值为1、2、3，则红柱面积为6；一段绿柱数值为-1、-2、-3，则绿柱面积为-6（或绝对值6）。 ‌

- 积分近似法‌

  从数学角度，可将MACD柱状图视为离散曲线，通过积分计算其与零轴的面积。实际操作中，直接累加法是积分的离散化近似，兼顾了准确性与简便性。 ‌

- 软件自动计算‌

  多数专业股票分析软件（如通达信、同花顺等）提供自定义指标功能，可通过编写公式自动计算并显示MACD面积。 ‌

#### 3.1.2 分析价值与应用

MACD面积的核心价值在于‌量化动能变化‌，尤其用于识别趋势强度与潜在反转信号：

- 趋势强度判断‌

  红柱面积持续增大，表明多头动能强劲，上涨趋势可能延续；绿柱面积持续增大，则表明空头动能占优，下跌趋势可能持续。 ‌

- 动能衰竭预警（面积萎缩）‌

  当股价创新高，但红柱面积较前一波明显缩小，形成“顶背离”，预示上涨动能不足，可能见顶回落；反之，股价创新低但绿柱面积缩小，形成“底背离”，预示下跌动能衰竭，可能见底反弹。 ‌
  
- 多空力量对比‌
  
  通过比较红柱面积与绿柱面积的相对大小，可判断当前市场主导力量。例如，红柱面积远大于绿柱面积，说明多方占优。 ‌
